...
class WorldModel:
    ...
    """ Calculates the change in boat location and wind conditions and updates
        the model accordingly """
    def update_model(self):
        """ Calculate time since this function was last called """
        dt = time.time()-self.lastclock
        self.lastclock += dt # set lastclock for next run through here
        """ Calculate relative wind """
        self.relwind = abs(self.wind.windheading-self.boat1.heading)%(2.0*pi)
        if self.relwind > pi:
            self.relwind = 2.0*pi-self.relwind
        self.relwindcomp = pi-self.relwind
        """ Update boat location (i.e. play God and move the boat) """
...
class Boat:
    ...
    def __init__(self,length,xpos,ypos,color=(100,100,100)):
        ...
        self.xpos = xpos
        self.ypos = ypos
        self.MainPos = 0 # position of main sail
        #self.JibPos = 0
    ...
    """ Update boat state (incl. location) using the provided model """
    ...
    """ Adjust sails and rudder to suggestions if possible """
    def trim(self,model):
    ...
    """ Update kinematics (i.e. boat speed, heading, and position). """
...
#        floating log aspect (in cartesian, rather than doing vector addition, which is also possible)
    ...
    """ Get a string containing the current boat position, heading, and speed. """
    def posStr(self):
...
if __name__ == '__main__':
    ...
    running = True
    while running:
        """ While the simulation is running, keep updating the model """
        model.update_model()
        time.sleep(.1)
